jQuery(document).ready(function ($) {
        ...
        jQuery.each(response, function (status, message) {
            switch (status) {
                ...
                case 'group_limit':
                    ...
                    let current_seat_count = parseInt(jQuery('.ldgr_seats_left').val());
                    if (current_seat_count < parseInt(message)) {
                        jQuery('[name="ldgr_seats_left"]').attr('value', message);
                        let seats_left_msg = wdm_ajax.seats_left_text.replace('{seat_count}', message);
                        jQuery('.ldgr-seats-left-count').data('seat-count', message).text(seats_left_msg);
                    }
                    break;
                ...
                case 'total_limit':
                    ...
                    if ('on' === wdm_ajax.is_fix_group_limit) {
                        let current_total_count = parseInt(jQuery('.ldgr_total_seats').val());
    ...
    $('.ldgr_total_seats').on('focusout', function (event) {
        let seats_left = parseInt($('.ldgr-seats-left-count').data('seat-count'));
        ...
        if (total_seats < original_total_seats) {
            ...
            if (diff > seats_left) {
                ...
                jQuery('.ldgr-seats-left-count').data('seat-count', seats_left).text(seats_left_msg);
                // Update the original count;
    ...
    $('.ldgr-reset-seats').on('click', function () {
        ...
        let original_toal = parseInt(wdm_ajax.original_total_seats);
        if (total_seats === original_toal) {
            snackbar(wdm_ajax.no_change_reset_msg);
            return;
        }
        $('.ldgr_total_seats').val(wdm_ajax.original_total_seats).trigger('focusout');
        $(this).find('.dashicons-update').addClass('spin');
        setTimeout(function () {
            $('.ldgr-reset-seats').find('.dashicons-update').removeClass('spin');
